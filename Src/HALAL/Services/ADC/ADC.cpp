...
ADC::InitData::InitData(ADC_TypeDef* adc, uint32_t resolution,
...
ADC::Peripheral::Peripheral(ADC_HandleTypeDef* handle, LowPowerTimer& timer,
                            InitData& init_data)
    : handle(handle), timer(timer), init_data(init_data) {
    dma_data_buffer =
        (uint16_t*)MPUManager::allocate_non_cached_memory(2 * ADC_BUF_LEN);
}
...
void ADC::turn_on(uint8_t id) {
    ...
    if (HAL_ADC_Start_DMA(peripheral->handle,
        ...
        ErrorHandler("DMA - %d - of ADC - %d - did not start correctly",
                     peripheral->init_data.dma_stream, id);
        return;
    }
...
void ADC::init(Peripheral& peripheral) {
    ...
    for (uint32_t channel : peripheral.init_data.channels) {
        ...
        if (HAL_ADC_ConfigChannel(&adc_handle, &sConfig) != HAL_OK) {
            ErrorHandler("ADC ConfigChannel - %d - did not start correctly",
                         init_data.name);
        }
        counter++;
    }
