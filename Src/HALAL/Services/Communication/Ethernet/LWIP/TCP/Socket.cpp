...
Socket::Socket(IPV4 local_ip, uint32_t local_port, IPV4 remote_ip,
    ...
    : local_ip(local_ip), local_port(local_port), remote_ip(remote_ip),
      remote_port(remote_port), use_keep_alives{use_keep_alive} {
  ...
  connecting_sockets[remote_node] = this;
  tcp_connect(connection_control_block, &remote_ip.address, remote_port,
              connect_callback);
  OrderProtocol::sockets.push_back(this);
}
...
Socket::Socket(IPV4 local_ip, uint32_t local_port, IPV4 remote_ip,
  ...
  keepalive_config.inactivity_time_until_keepalive_ms =
      inactivity_time_until_keepalive_ms;
...
void Socket::send() {
  ...
  while (error == ERR_OK && !tx_packet_buffer.empty() &&
    ...
    error = tcp_write(socket_control_block, temporal_packet_buffer->payload,
                      temporal_packet_buffer->len, TCP_WRITE_FLAG_COPY);
    if (error == ERR_OK) {
      tx_packet_buffer.pop();
      tcp_output(socket_control_block);
      memp_free_pool(memp_pools[PBUF_POOL_MEMORY_DESC_POSITION],
                     temporal_packet_buffer);
    } else {
      if (error == ERR_MEM) {
        close();
        ErrorHandler(
            "Too many unacked messages on client socket, disconnecting...");
      } else {
...
err_t Socket::connect_callback(void *arg, struct tcp_pcb *client_control_block,
                               err_t error) {
  IPV4 remote_ip;
...
err_t Socket::receive_callback(void *arg, struct tcp_pcb *client_control_block,
                               struct pbuf *packet_buffer, err_t error) {
...
err_t Socket::poll_callback(void *arg, struct tcp_pcb *client_control_block) {
  ...
  if (socket != nullptr) {
    ...
    if (socket->state == CLOSING) {
      ...
      tcp_connect(socket->connection_control_block, &socket->remote_ip.address,
                  socket->remote_port, connect_callback);
    }
    return ERR_OK;
  } else {
...
err_t Socket::send_callback(void *arg, struct tcp_pcb *client_control_block,
                            uint16_t length) {
  Socket *socket = (Socket *)arg;
  socket->socket_control_block = client_control_block;
...
void Socket::error_callback(void *arg, err_t error) {
  Socket *socket = (Socket *)arg;
  socket->close();
  ErrorHandler("Client socket error: %d. Socket closed, remote ip: %s", error,
...
void Socket::connection_error_callback(void *arg, err_t error) {
  ...
  ErrorHandler("Connection socket error: %d. Couldn t start client socket ",
               error);
}

err_t Socket::connection_poll_callback(
    void *arg, struct tcp_pcb *connection_control_block) {
  Socket *socket = (Socket *)arg;
...
void Socket::config_keepalive(tcp_pcb *control_block, Socket *socket) {
  ...
  control_block->keep_idle =
      socket->keepalive_config.inactivity_time_until_keepalive_ms;
