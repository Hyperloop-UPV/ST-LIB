...
ServerSocket::ServerSocket(ServerSocket &&other)
    : local_ip(move(other.local_ip)), local_port(move(other.local_port)),
      state(other.state),
...
void ServerSocket::send() {
  ...
  while (error == ERR_OK && !tx_packet_buffer.empty() &&
    ...
    error = tcp_write(client_control_block, temporal_packet_buffer->payload,
...
err_t ServerSocket::receive_callback(void *arg,
...
err_t ServerSocket::poll_callback(void *arg,
                                  struct tcp_pcb *client_control_block) {
  ServerSocket *server_socket = (ServerSocket *)arg;
  server_socket->client_control_block = client_control_block;
...
err_t ServerSocket::send_callback(void *arg,
                                  struct tcp_pcb *client_control_block,
                                  u16_t len) {
  ServerSocket *server_socket = (ServerSocket *)arg;
  server_socket->client_control_block = client_control_block;
...
void ServerSocket::config_keepalive(tcp_pcb *control_block,
  ...
  control_block->keep_idle =
      server_socket->keepalive_config.inactivity_time_until_keepalive_ms;
  control_block->keep_intvl =
      server_socket->keepalive_config.space_between_tries_ms;
  control_block->keep_cnt =
      server_socket->keepalive_config.tries_until_disconnection;
}
