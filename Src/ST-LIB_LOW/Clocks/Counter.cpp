...
Counter::Counter(uint64_t update_period_ms) : update_period_ms(update_period_ms) {
    this->time_id =
        Time::register_low_precision_alarm(this->update_period_ms, [&]() { this->update(); });
}

Counter::~Counter() { Time::unregister_low_precision_alarm(this->time_id); }

void Counter::count() { this->counter++; }
