...
SensorInterrupt::SensorInterrupt(
    ...
    : SensorInterrupt::SensorInterrupt(
          pin,
          std::forward<std::function<void()>>(action),
          &value,
          trigger
      ) {}

void SensorInterrupt::read() { *value = (PinState)ExternalInterrupt::get_pin_value(id); }

uint8_t SensorInterrupt::get_id() { return id; }
