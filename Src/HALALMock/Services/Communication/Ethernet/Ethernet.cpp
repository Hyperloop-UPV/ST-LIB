...
void Ethernet::start(IPV4 local_ip, IPV4 subnet_mask, IPV4 gateway) {
    if (!is_running && is_ready) {
        ...
        is_running = true;
        LOG_INFO("Ethernet started succesfully");
    } else {
        LOG_ERROR("Unable to start Ethernet!");
    }

    if (not is_ready) {
        LOG_ERROR("Ethernet is not ready");
        return;
    }
}
...
void Ethernet::inscribe() {
    ...
    const static Pin pin_list_ethernet[number_pin_ethernet] = {
        PA1, PA2, PA7, PB13, PC1, PC4, PC5, PG11, PG0, PG13};
    if (!is_ready) {
        for (size_t i = 0; i < number_pin_ethernet; i++) {
            ...
            if (pin_data.type == PinType::NOT_USED) {
                ...
                LOG_DEBUG("Pin subscribed succcesfully to Ethernet");
            } else {
                LOG_ERROR(
                    "Error inscribing ethernet Pins, "
                    "PA1,PA2,PA7,PB13,PC1,PC4,PC5,PG11,PG0,PG13 must be free");
                return;
            }
        }
        is_ready = true;
...
void Ethernet::update() {
    // I'm going to leave the case is not running so it warn the user to check
    // if you have done inscribed
    if (not is_running) {
        LOG_ERROR("Ethernet is not running, check if its been inscribed");
        return;
    }
}
