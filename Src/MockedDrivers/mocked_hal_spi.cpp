...
extern "C" HAL_StatusTypeDef HAL_SPI_Receive(SPI_HandleTypeDef *hspi, uint8_t *pData,
                                              uint16_t Size, uint32_t Timeout) {
  (void)Timeout;
...
extern "C" HAL_StatusTypeDef HAL_SPI_TransmitReceive(SPI_HandleTypeDef *hspi, uint8_t *pTxData,
...
extern "C" HAL_StatusTypeDef HAL_SPI_Transmit_DMA(SPI_HandleTypeDef *hspi, uint8_t *pData,
                                                   uint16_t Size) {
...
extern "C" HAL_StatusTypeDef HAL_SPI_TransmitReceive_DMA(SPI_HandleTypeDef *hspi,
                                                          uint8_t *pTxData,
                                                          uint8_t *pRxData,
                                                          uint16_t Size) {
  g_state.calls[static_cast<std::size_t>(
      ST_LIB::MockedHAL::SPIOperation::TransmitReceiveDMA)]++;
  g_state.last_handle = hspi;
  g_state.last_size_words = Size;
  const auto size_bytes = static_cast<std::size_t>(Size) * frame_bytes(hspi);
  store_tx(pTxData, size_bytes);
