...
struct EXTIDomain {
    ...
    struct Device {
        ...
        consteval Device(ST_LIB::GPIODomain::Pin pin, Trigger trigger, void (*action)())
            : pin(pin, static_cast<ST_LIB::GPIODomain::OperationMode>(trigger),
                  ST_LIB::GPIODomain::Pull::None, ST_LIB::GPIODomain::Speed::Low),
              action(action) {
        ...
        for (std::size_t i = 0; i < N; i++) {
            ...
            if (used_lines_mask & line_mask) {
                ST_LIB::compile_error(
                    "EXTI line already used (Hardware Conflict: Pins with same number share EXTI "
                    "Line)");
            }
