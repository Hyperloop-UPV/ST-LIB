...
public:
    ...
    /**
     * @brief function that sets a deadtime, in which the PWM wouldn t be on HIGH no matter the duty
     * cycle
     *
     * 	This function has to be called while the PWM is turned off.
     * 	This function actually substracts from the HIGH state of the PWM the amount of ns, pulling
     * it down; thus effectively reducing the duty cycle by an amount dependant on the frequency and
     * the dead time.
     */
