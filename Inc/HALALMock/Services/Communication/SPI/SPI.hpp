...
/**
 * @brief SPI service class. Abstracts the use of the SPI service of the HAL
 * library.
 *
 */
class SPI {
   public:
    ...
    /**
     * @brief Struct which defines all data referring to SPI peripherals. DO NOT
     * MODIFY ON RUN TIME. declared public so HAL callbacks can directly access
     * the instance NOT intended to be modified after the inscribe of the spi,
     * configuration should be made in Runes.hpp
     *
     */
    struct Instance {
        Pin* SCK;  /**< Clock pin. */
        ...
        bool initialized =
            false; /**< Peripheral has already been initialized */

        bool use_DMA = false;
        bool using_ready_slave = false;
        string name;
        SPIstate state =
            IDLE; /**< State of the spi on the Order communication*/
        uint8_t* rx_buffer;
        uint8_t* tx_buffer;
        uint16_t* available_end; /**< variable that checks for what Order id is
                                    the other end ready*/
        ...
        uint64_t try_count = 0; /**< Tries from the master to communicate a packet with the slave
                  for debugging (affected by how much the slave delays on
                  preparing a packet)*/
    ...
    /**
     * @brief Enum that abstracts the use of the Instance struct to facilitate
     * the mocking of the HALAL.
     *
     */
    enum Peripheral {
        peripheral1 = 0,
     ...
     * 			    Returns false if a problem has occurred.
     */
    static bool transmit_and_receive(uint8_t id, span<uint8_t> command_data,
    ...
    /**
     * @brief slave listen Orders method. When called, the slave will start to
     * listen Orders until the state is set again to IDLE
     */
     ...
     * 			    Returns false if a problem has occurred.
     */
    static bool transmit_and_receive_DMA(uint8_t id, span<uint8_t> command_data,
                                         span<uint8_t> receive_data);
    /**
     * @brief method that needs to be called periodically by the master code for
     * proper communication. Can be called at any speed, but the faster it is
     * called, the faster the Orders will resolve.
     *
     * this update has to be called by the code in order for master to check if
     * the slave is ready to send the Order. If it is not called periodically,
     * the master_transmit_Order will not work. Not needed for dummy
     * communication (not using Orders)
     */
    ...
    /**
     * @brief master Order that checks the state of the slave, used on the
     * callback of the SPI module and handled automatically by Order_update
     * method (as long as it is called).
     */
    static void master_check_available_end(SPI::Instance* spi);

    /**
     * @brief slave updates the information on its buffer to check what ID is
     * master asking on next query
     */
    static void slave_check_packet_ID(SPI::Instance* spi);

    /**
     * @brief This method sets chip select to high level, and is used
     * automatically by the SPI module
     *
     * The high level signal on SPI on chip select marks a slave to not process
     * anything. For more information read the chip_select_off method.
     *
    ...
    /**
     * @brief This method sets chip select to low level, and is used
     * automatically by the SPI module.
     *
     * The low level signal on SPI on chip select marks the slave connected to
     * it to start processing data, and is marked by the master The slave on the
     * ST-LIB doesn t use this configuration yet and is instead active all the
     * time, so for now it can only function as a single point to point
     * configuration to have multiple ST-LIB SPI slaves on a single SPI
     * interface you may need to implement an EXTI on the slave that deactivates
     * it while its on. This method is still used to communicate to any slave
     * that is not coded with the ST-LIB, as those follow the common rules of
     * the SPI
     *
     * @param spi Id of the SPI
     */
    ...
    static inline void spi_communicate_order_data(SPI::Instance* spi,
    ...
    /*
     * @brief returns true if its KNOWN that the slave has the packet ready, and
     * false if it is NOT KNOWN
     *
     * This function uses the optional RS to know if the slave is ready, and
     * always returns false if the RS is not used.
     */
    static bool known_slave_ready(SPI::Instance* spi);

    /**
     * @brief function used by slave to signal using RS that it has the message
     * ready
     */
    static void mark_slave_ready(SPI::Instance* spi);

    /**
     * @brief function used by slave to signal using RS that it is waiting a new
     * message
     */
    static void mark_slave_waiting(SPI::Instance* spi);

    /**
     * @brief Recovers SPI from any error so it starts working again, and counts
     * the error
     */
   ...
   private:
    /**
     * @brief This method initializes the SPI peripheral that is passed to it as
     * a parameter.
     *
    ...
    static unordered_map<uint8_t, std::pair<int, int>>
        spi_master_sockets; /** Map that associates registered SPI master with
                               its server socket and its port */
