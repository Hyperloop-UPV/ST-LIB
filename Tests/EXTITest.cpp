...
void test_interrupts_1(EmulatedPin &pin1, EmulatedPin &pin2)
{

    std::this_thread::sleep_for(std::chrono::milliseconds(500));

    std::unique_lock<std::mutex> lock(ExternalInterrupt::mutex);
    pin1.PinData.exti.trigger_signal = true;
    lock.unlock();

    std::this_thread::sleep_for(std::chrono::milliseconds(500));
    lock.lock();
    pin2.PinData.exti.trigger_signal = true;
    lock.unlock();
    std::this_thread::sleep_for(std::chrono::milliseconds(500));
...
TEST(EXTITest, TurnOn){
    ...
    EmulatedPin &pin = SharedMemory::get_pin(PE0);

    pin.type = PinType::EXTIPin; //Just for testing, SharedMemory is reinit in each test

    ASSERT_FALSE(pin.PinData.exti.is_on);

    ExternalInterrupt::turn_on(id1);

    ASSERT_TRUE(pin.PinData.exti.is_on);
...
TEST(EXTITest, get_pin_value){
    ...
    ASSERT_FALSE(signal);
    pin.PinData.exti.trigger_signal = PinState::ON;
...
TEST(EXTITest, handle_interrupt_1){
    ...
    EmulatedPin &pin2 = SharedMemory::get_pin(PE1);
    pin2.PinData.exti.trigger_signal = PinState::ON;
...
TEST(EXTITest, handle_interrupt_concurrece){
    ...
    SharedMemory::start("GPIO_Name", "State_Machine_Name");
    EmulatedPin &pin1 = SharedMemory::get_pin(PE0);
    pin1.PinData.exti.trigger_signal = PinState::OFF;

    EmulatedPin &pin2 = SharedMemory::get_pin(PE1);
    pin2.PinData.exti.trigger_signal = PinState::OFF;
    ...
    std::thread sim_interrupts(test_interrupts_1, std::ref(pin1), std::ref(pin2));
    sim_interrupts.join();
    ExternalInterrupt::stop();
    //check if modify global variables
    ASSERT_TRUE(enter_on_callback_1);
